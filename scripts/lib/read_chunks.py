...
def readHead(title, active_url, prefix=""):
    if prefix != "":
        prefix = prefix + "-";
    page_name = active_url.replace(".html", "");
    headfile = "../html-chunks/" + prefix + "head.html";
    return open(headfile, "r").read().replace("TMPTITLE", title).replace("PAGECSS", page_name);

def readNav(active_url, prefix=""):
    if prefix != "":
        prefix = prefix + "-";
    navfile = "../html-chunks/" + prefix + "nav.html";
    navlines = open(navfile, "r").readlines()
    for x in range(len(navlines)):
        if active_url not in ["index.html", "servers.html"]:
